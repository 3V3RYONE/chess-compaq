...
// FINALLLLLLLLLLLLLLLLLLLLLLLLLLLLL GAMEEEe
...
#define YPP A2  // must be an analog pin, use "An" notation!
#define XMM A3  // must be an analog pin, use "An" notation!
#define YMM 8   // can be a digital pin
#define XPP 9   // can be a digital pin
...
TouchScreen TSS = TouchScreen(XPP, YPP, XMM, YMM, 300);
...
void loop2() 
{
  TSPoint p = TSS.getPoint();  //Get touch point
...
void playerMove()
{
  ...
  do
  {    
    p = TSS.getPoint();  //Get touch point  
    if (p.z > TSS.pressureThreshhold)
    {
      p.x = map(p.x, TS_MAXX, TS_MINX, 0, 320);
      p.y = map(p.y, TS_MAXY, TS_MINY, 0, 240);
   ...
   }while(p.z<TSS.pressureThreshhold); 
}
